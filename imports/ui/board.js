...
export default class Board {
	...
	draw() {
		  ...
		  this.drawMowerHistory(groups, scales, [this.start]);
	}
  ...
  buildMap(gridSize, ratios) {
   ...
   executeCommands(e) {
    ...
    for(i = 0; i < content.length; i++) {
      var next = this.getNext(map, current, content[i]);
      switch(next.type) {
        case "grass":
          path.push(next);
          current = next;
        ...
        case "rock":
          ...
          break;
        case "lava":
          this.drawMowerHistory(groups, scales, path);
          alert("The mower turned into ashes, as predicted.", "Start again.");
          $('#commands').val("");
          this.drawMowerHistory(groups, scales, [start]);
          return;
        default:
